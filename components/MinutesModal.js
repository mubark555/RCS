...
// تصدير المحضر كمستند قابل للطباعة/الحفظ PDF
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  if (!w) {
    alert("فضلاً اسمح بالنوافذ المنبثقة لتصدير المحضر.");
    return;
  }
...
export default function MinutesModal({ meeting, onClose, onEdit, readOnly }) {
  const m = meeting;
  ...
  return (
    <Modal title="محضر الاجتماع" onClose={onClose}>
      ...
      <div className="modal-actions" style={{ marginTop: 18 }}>
        <button className="btn primary" onClick={() => exportMinutes(m)}>
          <Icon name="upload" size={16} /> تصدير المحضر (PDF)
        </button>
